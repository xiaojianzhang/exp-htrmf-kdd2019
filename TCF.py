...
class TCF(object):
    ...
    def _AR_X_regularizer(self):
        ...
        m = 2
        for t in range(m-1, self.T):
            self.AR_X_norm += LA.norm(self.X[:,t] - self.X[:,t-1])**2.0
...
if __name__ == "__main__":
    ...
    model = TCF(Y=Y_train, imputation_value_x_coor=imputation_value_x_coor, imputation_value_y_coor=imputation_value_y_coor,
                 forecasting_x_coor=forecasting_x_coor, forecasting_y_coor=forecasting_y_coor,
                 Omega=Omega, Y_size=(n,T),
                 latent_dim=30, lambda_f=5, lambda_x=10,
