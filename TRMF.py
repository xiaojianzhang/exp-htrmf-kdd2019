...
class TRMF(object):
    ...
    def __init__(self, Y, imputation_value_x_coor, imputation_value_y_coor, forecasting_x_coor, forecasting_y_coor, Omega, Y_size, lagset, latent_dim, lambda_f, lambda_x, lambda_w, eta, max_iterations, forecasting_horizon=4, seed=None, true_imputation_value=None, true_forecasting_value=None):
        """
        ...
        self.lagset = lagset
        self.lagset.sort()
        self.lagset_len = len(self.lagset) #the length of the lag set
        self.max_lag = max(self.lagset) #the largest lag
        self.lag2index = self._lag2index(self.lagset)
        self.lagset_union_zero = [0] + self.lagset #the union of the lag set and {0}.
        ...
        self.W = np.zeros([self.k, self.lagset_len])
        self._update_W()
    ...
    def _update_X(self):
        ...
        for t in range(m-1, self.T):
            ...
            for j in range(self.T):
                l = t - j
                if l in self.lagset_union_zero:
                    if l == 0:
                        W_t_bar.append(np.eye(self.k))
                    else:
                        W_t_bar.append(-np.diag(self.W[:,self.lag2index[l]]))
                else:
    ...
    def _update_W(self):
        ...
        for row in range(self.k):
            ...
            X_r_bar = []
            for t in range(m-1, self.T):
                x_r_m_bar = [self.X[row, t-self.lag2index[l]] for l in self.lagset]
                X_r_bar.append(x_r_m_bar)

            X_r_bar = np.array(X_r_bar)

            #solve for W_r
            A = 2*float(self.lambda_w)/float(self.lambda_x) * np.eye(self.lagset_len) + np.dot(X_r_bar.T, X_r_bar)
            b = np.dot(X_r_bar.T, x_r_bar)
            w_r = np.linalg.solve(A, b)
            assert np.allclose(np.dot(A, w_r), b), "update W: fail"
            self.W[row,:] = w_r
    ...
    def _AR_X_regularizer(self):
        ...
        m = self.max_lag + 1
        for t in range(m-1, self.T):
            self.AR_X_norm += np.sum((self.X[:,t] - sum([np.diag(self.W[:,self.lag2index[l]]).dot(self.X[:,t-l]) for l in self.lagset]))**2.0)
    ...
    def _forecasting_error(self):
        ...
        for t in range(self.forecasting_horizon):
            new_x = sum([np.diag(self.W[:,self.lag2index[l]]).dot(X[:,-l]) for l in self.lagset])
...
if __name__ == "__main__":
    ...
    model = TRMF(Y=Y_train, imputation_value_x_coor=imputation_value_x_coor, imputation_value_y_coor=imputation_value_y_coor,
                 forecasting_x_coor=forecasting_x_coor, forecasting_y_coor=forecasting_y_coor,
                 Omega=Omega, Y_size=(n,T), lagset=[1,2,3,4,5,6,7],
                 latent_dim=15, lambda_f=5, lambda_x=10, lambda_w=5,
